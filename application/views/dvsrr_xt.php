<main>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table sm_checks compact" id="dataTableXT" width="100%" cellspacing="0">
                        <thead>
                            <tr align="center">
                                <th>Payment Date</th>
                                <th>SI No.</th>
                        ...
                        <tbody>
                            ...
                            <?php 
                                ...
                                unset($rows); // Break reference to avoid unintended modifications
                                
                                // Loop through grouped data
                                foreach ($grouped_data as $pay_date => $rows) {
                                    $sub_total_net = 0; // Initialize subtotal for the current date
                                    
                                    // echo "<tr  class='group-row' align='left'>
                                    //         <td colspan='10'><strong>{$pay_date}</strong></td>
                                            
                                    //     </tr>";
                                
                                    foreach ($rows as $row2) {
                                        $sub_total_net += $row2['net_amount'];
                                        $grand_total_net += $row2['net_amount'];
                                    ?>
                                    <tr>
                                        ...
                                        <td><?php echo number_format($row2['deals'], 2); ?></td>
                                        <td><?php echo number_format($row2['disc_amount'], 2); ?></td>
                                        <td><?php echo number_format($row2['tax_amount'], 2); ?></td>
                                        <td><?php echo number_format($row2['cash_amount'], 2); ?></td>
                                        <td><?php echo number_format($row2['check_amount'], 2); ?></td>
                                        <td><?php echo number_format($row2['net_amount'], 2); ?></td>
                        ...
                        <tfoot>
                            ...
                            <tr >
                                ...
                                <td colspan="10"><strong>Gross Amount</strong></td>
                                <td><strong><?php echo number_format(@$grand_total_net, 2); ?></strong>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="10"><strong> Total Incentives Amount</strong></td>
                                <td><strong><?php echo number_format(@$incentives->inc_used, 2); ?></strong></td>


                            </tr>

                            <tr>
                                <td colspan="10"><strong> Total Satellite Amount</strong></td>
                                <td><strong><?php echo number_format(@$satellite->total_appr_amount, 2); ?></strong></td>


                            </tr>

                            <tr>
                                <td colspan="10"><strong> Total Palawan Amount</strong></td>
							...
							<tr>
                                <td colspan="10"><strong> Total BO Amount</strong></td>
                                <td><strong><?php echo number_format(@$incentives->total_bo_amount, 2); ?></strong></td>


                            </tr>

                            <tr>
                                <td colspan="10"><strong> Total UTC Amount</strong></td>
                                <td><strong><?php echo number_format(@$utc->total_utc, 2); ?></strong></td>


                            </tr>

                            <tr>
                                <td colspan="10"><strong>Net Amount</strong></td>
                                <td>
                                    <strong>
                                        <?php
                                            $grandTotalNet   = isset($grand_total_net) ? $grand_total_net : 0;
                                            $incUsed         = isset($incentives->inc_used) ? $incentives->inc_used : 0;
                                            $satelliteAmount = isset($satellite->total_appr_amount) ? $satellite->total_appr_amount : 0;
                                            $palawanAmount   = isset($palawan->total_pay_amount) ? $palawan->total_pay_amount : 0;
                                            $utcAmount       = isset($utc->total_utc) ? $utc->total_utc : 0;
                                            $boAmount        = isset($incentives->total_bo_amount) ? $incentives->total_bo_amount : 0;

                                            $netAmount = $grandTotalNet - $incUsed - $satelliteAmount - $palawanAmount - $utcAmount + $boAmount;
    ...
    $(document).ready(function() {
        ...
        var table = $('#dataTableXT').DataTable({
            ...
            buttons: [
                    ...
                    customize: function(doc) {
                        ...
                        if (doc.content && Array.isArray(doc.content[1].table.body)) {
                            ...
                            if (body.length > 6) {
                                ...
                                const secondLastRow = body[body.length - 7];

                                if (lastRow[0].text.includes("Net Amount")) {
                                    tfootData = body.splice(-7,
                                    7); // Remove last two rows (tfoot) and store
                                }
                            }
                            ...
                            body.forEach((row, index) => {
                                ...
                                const netAmount = parseFloat(row[10].text.replace(/,/g,
                                    ''));
                                ...
                                if (lastPayDate !== payDate) {
                                    ...
                                    if (!isFirstGroup && !isNaN(subTotalNet) &&
                                        ...
                                        newBody.push([{
                                            ...
                                            {}, {}, {}, {}, {}, {}, {}, {}, {},
                                            {
                                                text: number_format(subTotalNet,
                                                    2),
                                                alignment: 'right',
                                                bold: true
                                            }
                ...
                },
                {
                    ...
                    customize: function(csv) {
                        ...
                        var tfootRows = data.slice(-5);
                        var dataRows = data.slice(0, -5); // Exclude last two rows
                        ...
                        dataRows.forEach((row, index) => {
                            if (index === 0) {
                                ...
                                var netAmountText = cells[10]?.replace(/"/g, "").trim();
                ...
                //             var cells = $('c', this);
                //             var payDate = cells.eq(0).text();
                //             var netAmount = parseFloat(cells.eq(10).text()) || 0;

                //             if (payDate !== lastPayDate) {
                //                 if (lastPayDate) {
                //                     newRows.push(createSubtotalRow(subTotalNet)); // Add subtotal row
                //                 }
                //                 newRows.push(createGroupRow(payDate)); // Add group row
                //                 subTotalNet = 0; // Reset subtotal
                //             }

                //             subTotalNet += netAmount;
                //             newRows.push(this); // Add the actual data row
                //             lastPayDate = payDate;
                //         });

                //         if (lastPayDate) {
                //             newRows.push(createSubtotalRow(subTotalNet)); // Add the last subtotal row
                //         }

                //         $('sheetData', sheet).append(newRows);
                //     }
                // }
            ...
            drawCallback: function(settings) {
                ...
                api.column(0, {
                    ...
                    .each(function(pay_date, i) {
                        ...
                        var netAmount = parseFloat(row.find('td').eq(10).text().replace(/,/g,
                            '')); // Get net_amount from the row (11th column)
                        ...
                        if (lastPayDate !== pay_date) {
                            ...
                            if (lastPayDate !== null) {
                                $(rows).eq(i - 1).after(
                                    '<tr class="sub-total-row"><td colspan="10"><strong>Subtotal </strong></td><td><strong>' +
                            ...
                            $(rows).eq(i)
                                .before('<tr class="group-row"><td colspan="10"><strong>' +
                                    pay_date + '</strong></td></tr>');
                        } else {
                            // Accumulate the net_amount for the current pay_date group
                            subTotalNet += netAmount;
                        }
