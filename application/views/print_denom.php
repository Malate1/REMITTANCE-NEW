<?php
    ...
    $pdf->setx('135');
    $pdf->Cell(290,-25,"Total Collection :",0,1,'L');
    $pdf->setx('170');
    $pdf->Cell(348,25,number_format($result->total_remittance,2),0,1,'L');

    $total_amount = $result->expenses_amt + $result->total_collection;
    ...
    if($result->expenses==""){
        ...
        $exp = $result->expenses_amt;
    }

    $pdf->SetFont('Arial','B',11);
    $pdf->Ln('1');
    $pdf->Cell(27,-10,"Expenses : ",0,1,'R');
    $pdf->setx('35');
    $pdf->Cell(90,10,$exp,0,1,'L');
